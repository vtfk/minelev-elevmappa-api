...
async function newUPNtoOldSAM (upn) {
  const url = `https://identities.api.minelev.no/identities/upn/${upn}`
  const token = generateToken({ secret: process.env.BUDDY_JWT_SECRET })
...
module.exports.getStudents = async (request, response) => {
  ...
  const username = await newUPNtoOldSAM(validatedToken.upn)
...
module.exports.getStudent = async (request, response) => {
  ...
  try {
    ...
    if (!checkRelation) throw Error('Ingen tilgang. Lærer/Elev relasjon finnes ikke i Extens.')
