...
const upnMailToSam = require('./upn-mail-to.sam')

const upnToSam = upn => upn.split('@')[0]

/*
 * Quick fix. If user logs inn with mail adress.
 * Call identities.api.minelev.no to get SAM account name
*/
const getUsername = async upn => {
  const sam = upnToSam(upn)
  if (sam.includes('.')) {
    return upnMailToSam(upn)
  }
  return sam
}
